...
sub kvmkeymaplist {
    #<<< keep this short, it's static info.
    return [
        "de", "de-ch", "da", "en-gb", "en-us", "es", "fi", "fr", "fr-be", "fr-ca", "fr-ch", "hu",
        "is", "it", "ja", "lt", "mk", "nl", "no", "pl", "pt", "pt-br", "sv", "sl", "tr",
    ];
    #>>>
}
...
sub shell_quote {
    my ($str) = @_;
    return PVE::Cmd::shell_quote($str);
}
...
sub cmd2string {
    my ($cmd) = @_;
    return PVE::Cmd::to_string($cmd);
}

# split an shell argument string into an array,
sub split_args {
    my ($str) = @_;
    return PVE::Cmd::split_args($str);
}
