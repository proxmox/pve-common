...
sub active_workers {
    ...
    my $res = PVE::Tools::lock_file(
        "/var/log/pve/tasks/.active.lock",
        $timeout,
        sub {
...
sub fork_worker {
    ...
    local $SIG{INT} = local $SIG{QUIT} = local $SIG{PIPE} = local $SIG{TERM} = 'IGNORE';
    ...
    if (!$cpid) { # child
        ...
        if ($sync) {
            POSIX::close($resfh);
            if (-t STDIN) {
                POSIX::tcsetpgrp(fileno(STDIN), $ppgid)
                    or die "failed to tcsetpgrp to parent: $!\n";
            }
        }
